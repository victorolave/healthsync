# agenda-persistence — Delta Specification
...
This spec covers NEW capability only. It does not repeat Phase 1 domain invariants
(see `openspec/specs/scheduling-domain/spec.md`).
    ...
    messages/
      infrastructure/
        prisma/
          ...
          prisma-agenda.repository.spec.ts # Integration tests (requires DATABASE_URL)
          mappers/
            working-hours.mapper.ts
...
cd apps/scheduling && DATABASE_URL=... pnpm test:integration
```

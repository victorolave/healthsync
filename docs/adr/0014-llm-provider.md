...
scheduling-side 5 s AbortController window (ADR-0007), and be testable offline.
...
Timeout: **4.0 s** (configured in `Settings.request_timeout`), strictly below the 5 s
scheduling timeout to ensure a clean 503 from the Language service before the scheduling
